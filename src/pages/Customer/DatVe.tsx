...
export default function BusSeatSelection() {
    ...
    /* ── Seat generation — truyền bookedSeatLabels vào để đánh dấu booked ── */
        ...
        for (let row = 1; row <= rows; row++) {
            ...
            columns.forEach((col: any, colIndex: number) => {
                ...
                for (let i = 0; i < seatsInColumn; i++) {
                    ...
                    // ── Đánh dấu booked nếu label nằm trong bookedSeatLabels ──
    ...
    // Re-generate khi bookedSeatLabels thay đổi
    ...
    const getSeatStatus = (seat: Seat): SeatStatus => {
        if (seat.status === "booked") return "booked"; // ưu tiên booked từ BE
                                    ...
                                    <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-5">
                                        ...
                                        <div className="space-y-3">
                                            ...
                                            <div className="relative">
                                                ...
                                                <select value={selectedPickupId} onChange={e => { const chosenId = e.target.value; setSelectedPickupId(chosenId); const found = pickupPoints.find(p => p._id === chosenId); setSelectedPickupStopId(found?.stop_id._id ?? ""); }} disabled={pickupPoints.length === 0}
                                        ...
                                        <div className="space-y-3">
                                            ...
                                            <div className="relative">
                                                ...
                                                <select value={selectedDropoffId} onChange={e => { const chosenId = e.target.value; setSelectedDropoffId(chosenId); const found = dropoffPoints.find(p => p._id === chosenId); setSelectedDropoffStopId(found?.stop_id._id ?? ""); }} disabled={!selectedPickupId || dropoffPoints.length === 0}
                                            ...
                                            <div className="flex items-center justify-between gap-4 px-5 py-4 bg-gradient-to-r from-orange-500 to-orange-600 rounded-2xl shadow-lg">
                                                <div className="flex items-center gap-3">
                                                    ...
                                                    <div>
                                                        ...
                                                        <p className="text-2xl font-black text-white leading-tight">{ticketPrice.toLocaleString("vi-VN")}₫</p>
                                                    </div>
                                                </div>
                                                {selectedSeats.length > 0 && (
                                                    <div className="text-right flex-shrink-0">
                                                        ...
                                                        <p className="text-xl font-black text-white leading-tight">{(ticketPrice * selectedSeats.length).toLocaleString("vi-VN")}₫</p>
                                                    </div>
                                                )}
                                            </div>
                                        ) : null}
