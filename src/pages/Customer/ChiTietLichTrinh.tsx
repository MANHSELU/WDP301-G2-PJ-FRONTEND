...
function ScheduleAccordion({ trip }: { trip: Trip }) {
  const sortedStops = [...trip.time].sort(
  ...
  const calcArrival = (departureTime: string, totalHours: number) => {
    const base = new Date(
      new Date(departureTime).getTime() + totalHours * 60 * 60 * 1000
    );
    const time = base.toLocaleTimeString("vi-VN", {
      hour: "2-digit",
      minute: "2-digit",
    });
        ...
        <div className="relative">
          ...
          <div
            ...
            style={{
              left: "15px",
              background:
                "linear-gradient(to bottom, #fb923c, #fdba74, #fb923c)",
            }}
          />
          ...
          <div className="space-y-1">
            {sortedStops.map((stop, idx) => {
              ...
              const estHours =
                stop.estimated_time ?? (stop as any)[" estimated_time"] ?? 0;
              // ✅ Dùng cumulative hours để tính giờ đến đúng
              const { time, date } = calcArrival(
                trip.departure_time,
                cumulativeHours[idx]
              );
              ...
              return (
                ...
                >
                  ...
                  <div className="relative z-10 flex-shrink-0 mt-0.5">
                    ...
                    >
                      <MapPin
                        size={14}
                        className={
                          isFirst || isLast ? "text-white" : "text-orange-500"
                        }
                  ...
                  {/* Content */}
                  <div
                    className={`flex-1 flex items-start justify-between min-w-0 pb-4 last:pb-0 ${
                      !isLast ? "border-b border-orange-50" : ""
                    }`}
                  >
                    <div>
                      <p
                    ...
                    <div className="text-right flex-shrink-0 ml-4">
                      <p className="font-black text-base text-slate-800 tabular-nums">
                        {time}
                      </p>
  ...
  return (
    <div className="relative min-h-screen overflow-hidden bg-gradient-to-br from-slate-50 via-orange-50/30 to-slate-100">
      ...
      <div className="relative z-20 mx-auto flex min-h-[520px] w-full max-w-[1240px] items-center px-4 pb-16 pt-24 lg:min-h-[580px] lg:pt-20">
        <div className="page-enter-copy relative isolate -ml-8 max-w-[760px] space-y-6 sm:-ml-14 lg:-ml-24 w-full">
          ...
          <h1 className="hero-title relative z-10 py-1 text-[48px] font-black leading-[1.05] tracking-[-0.03em] text-[#0d142a] sm:text-[58px] lg:text-[72px] text-center md:text-left">
          ...
          <p className="relative z-10 max-w-[510px] text-base leading-relaxed text-[#475569] lg:text-lg text-center md:text-left">
      ...
      <div className="relative z-20 py-8 pb-20">
        <div className="max-w-7xl mx-auto px-4">
          <div className="grid grid-cols-1 lg:grid-cols-[320px_1fr] gap-8">
            ...
            {/* TRIP RESULTS */}
            <main className="space-y-5 order-1 lg:order-2">
                    ...
                    <div
                      className={`bg-gradient-to-br from-white via-white to-orange-50/30 rounded-2xl shadow-xl border-2 transition-all duration-300 relative overflow-hidden ${
                        isOpen
                          ...
                          : "border-orange-100/50 hover:shadow-2xl hover:border-orange-200 hover:-translate-y-1"
                      }`}
                    >
                      {/* Top accent */}
                      <div
                        className={`absolute top-0 left-0 right-0 h-1.5 bg-gradient-to-r from-orange-400 via-orange-500 to-orange-600 transition-opacity duration-300 ${
                          isOpen
                            ? "opacity-100"
                      ...
                      <div className="p-7">
                        ...
                        <div className="flex flex-col md:flex-row items-start md:items-center mb-6 gap-6">
                          <div className="flex-1 flex items-start md:items-center gap-6">
                            ...
                            <div className="flex flex-col md:flex-row items-center px-2 md:px-8 relative">
                        ...
                        {/* Bottom row */}
                        <div className="flex flex-col md:flex-row items-start md:items-center justify-between pt-5 border-t-2 border-orange-100 gap-4">
                          ...
                          <div className="w-full md:w-auto flex justify-start md:justify-end">
                            <button
                              onClick={() =>
                                DatVe(trip?.bus_id?.bus_type_id?._id)
                              }
                              className="bg-gradient-to-br from-orange-500 via-orange-600 to-orange-700 hover:from-orange-600 hover:via-orange-700 hover:to-orange-800 text-white px-8 py-3 rounded-xl font-bold text-base shadow-xl hover:shadow-2xl hover:shadow-orange-500/50 hover:scale-105 transition-all duration-300 relative overflow-hidden group/btn"
                            >
      ...
      <style>{`
        ...
        .bus-driver-fit { position: absolute; left: 26.3%; top: 30.7%; width: 11.6%; height: 15.8%; overflow: hidden; clip-path: polygon(8% 1%, 96% 5%, 100% 95%, 22% 98%, 2% 56%); transform: perspective(760px) rotateY(-12deg) rotate(-0.55deg); transform-origin: 54% 50%; box-shadow: inset 0 -14px 16px rgba(2,6,23,0.28); animation: bus-driver-settle 1.9s cubic-bezier(0.36, 0.06, 0.29, 0.97) infinite; }
        .bus-front-left-passenger { position: absolute; left: 48.4%; top: 26.2%; width: 11.6%; height: 15.6%; overflow: hidden; clip-path: polygon(18% 2%, 94% 6%, 98% 95%, 10% 97%, 4% 52%); transform: perspective(760px) rotateY(14deg) rotate(0.7deg); transform-origin: 50% 50%; box-shadow: inset 0 -14px 16px rgba(2,6,23,0.34); animation: bus-driver-settle 2s cubic-bezier(0.36, 0.06, 0.29, 0.97) infinite; z-index: 1; }
        ...
        .bus-driver-fit-img { position: absolute; left: -2%; top: 3%; width: 95%; height: 112%; object-fit: cover; object-position: center 8%; filter: saturate(0.82) contrast(1.08) brightness(0.9); opacity: 0.95; transform: scaleX(-1) rotate(5deg); animation: bus-driver-idle 1.65s cubic-bezier(0.36, 0.06, 0.29, 0.97) infinite; z-index: 1; }
        ...
        @media (max-width: 767px) {
          ...
          .page-enter-copy { padding-left: 12px; padding-right: 12px; }
          .bus-bob, .bus-aero-overlay, .bus-aero-trail { display: none !important; }
          .schedule-expand { padding-left: 0 !important; padding-right: 0 !important; }
          /* Make card buttons full width on small screens */
          .group/btn { width: 100%; }
        }
        ...
        @media (prefers-reduced-motion: reduce) {
          .bus-bob, .bus-cloud, .bus-tail-cloud, .bus-front-left-passenger, .bus-driver-fit { animation: none !important; transform: none !important; }
        }
      `}</style>
    </div>
  );
