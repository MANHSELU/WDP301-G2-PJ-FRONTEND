...
export default function BusBookingUI() {
  ...
  const seatList: string[] =
    selectedSeatLabels.length > 0 ? selectedSeatLabels : selectedSeats;
    ...
    for (let row = 1; row <= rows; row++) {
      ...
      columns.forEach((col: any, colIndex: number) => {
        ...
        for (let i = 0; i < seatsInColumn; i++) {
          ...
          const isSelected =
            selectedSeats.includes(id) || selectedSeatLabels.includes(label);
          seats.push({
            id,
            floor,
            row,
  ...
  const floor1Seats = useMemo(
    () => generateSeatsFromLayout(1),
    [trip, selectedSeats, selectedSeatLabels]
  );
  ...
  const handleBooking = async () => {
    ...
    try {
      ...
      const res = await fetch(
        "http://localhost:3000/api/customer/booking/create",
        {
          method: "POST",
          headers: {
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              ...
              <div className="bg-white rounded-lg p-8 shadow-lg border-2 border-orange-300">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  {[
                    {
                      value: "CASH_ON_BOARD",
                      label: "Trả tiền trên xe",
                      desc: "Thanh toán khi lên xe",
                      icon: "💵",
                    },
                    {
                      value: "ONLINE",
                    ...
                    >
                      ...
                      <div
                        className={`ml-auto mt-0.5 w-4 h-4 rounded-full border-2 flex-shrink-0 ${
                          paymentMethod === opt.value
                  ...
                  {[
                    {
                      ...
                      desc: "Hành khách bắt buộc phải mang theo giấy CMND/CCCD/Hộ chiếu hợp lệ khi lên xe.",
                    },
                    {
                      num: "2",
                      icon: "⏱️",
                      title: "Giờ Khởi Hành",
                      desc: "Xe khởi hành đúng theo giờ quy định. Quý khách vui lòng có mặt 15 phút trước.",
                    },
                    {
                      num: "3",
                      icon: "📍",
                      title: "Địa Điểm Xuất Phát",
                      desc: "Có mặt tại bến xe ít nhất 30 phút trước giờ khởi hành. Không chịu trách nhiệm cho khách lỡ chuyến.",
                    },
                    {
                      num: "4",
                      icon: "🎒",
                      title: "Quy Định Về Hành Lý",
                      desc: "Miễn phí: Tối đa 1 hành lý 50×40×25cm, không quá 15kg. Vượt quá sẽ tính phí bổ sung.",
                    },
                    {
                      num: "5",
                      icon: "⚠️",
                      title: "Giá Trị Tài Sản Cá Nhân",
                      desc: "Công ty không chịu trách nhiệm với điện thoại, laptop, tiền bạc, trang sức v.v...",
                    },
                  ].map((item) => (
                ...
                <div className="mt-5 pt-4 border-t-2 border-orange-300">
                  <label className="flex items-start gap-3 cursor-pointer">
                    ...
                    <span className="text-xs text-orange-900">
                      <strong>Tôi đã đọc và đồng ý</strong> với toàn bộ điều
                      khoản, lưu ý và chính sách của công ty vận tải.
                    </span>
                  </label>
                </div>
              </div>
            ...
            <div className="space-y-8 lg:sticky lg:top-6 h-fit">
              ...
              <button
                ...
                className={`w-full py-4 rounded-lg font-bold text-white transition-all text-lg shadow-lg flex items-center justify-center gap-2 ${
