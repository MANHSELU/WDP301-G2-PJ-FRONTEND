...
const PayBtn = ({
    ...
    <button
        onClick={onClick}
        className={`flex flex-col items-center justify-center gap-1.5 rounded-xl border-2 py-3 px-2 transition-all w-full ${selected ? "border-orange-400 bg-orange-50" : "border-gray-200 bg-white hover:border-orange-200"}`}
    >
...
const LegendItem = ({
    ...
    const v = seatVisualMap[status];
    return (
        <div className="flex items-center gap-2">
            <div className="relative h-[22px] w-[36px] flex-shrink-0">
                <span className={`pointer-events-none absolute left-[7px] top-[1px] h-[7px] w-[20px] rounded-t-[4px] border-[2px] border-b-0 ${v.detail}`} />
                <span className={`pointer-events-none absolute left-[3px] top-[6px] h-[10px] w-[29px] rounded-[3px] border-[2px] ${v.frame}`} />
                <span className={`pointer-events-none absolute left-[9px] top-[16px] h-[4px] w-[2px] rounded-b-[1px] ${v.leg}`} />
                <span className={`pointer-events-none absolute right-[9px] top-[16px] h-[4px] w-[2px] rounded-b-[1px] ${v.leg}`} />
            </div>
...
export default function TicketBooking() {
    ...
    const toggleSeat = (id: string) => {
        setSeats(prev =>
            prev.map(s => {
                if (s.id !== id) return s;
                if (s.status === "available") return { ...s, status: "selecting" as SeatStatus };
                if (s.status === "selecting") return { ...s, status: "available" as SeatStatus };
                return s;
            })
        );
    };
    ...
    const floorRows = (floorSeats: Seat[]): Seat[][] => {
        const rows: Seat[][] = [];
        for (let r = 0; r < 5; r++) {
                ...
                <div className="flex-1 space-y-5 min-w-0">
                    ...
                    <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-5">
                            ...
                            <div className="flex-1">
                                <div className="flex items-center gap-2 mb-3">
                                    <svg className="w-5 h-5 text-gray-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        ...
                                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 3v2m0 14v2M3 12h2m14 0h2M5.22 5.22l1.42 1.42m10.72 10.72 1.42 1.42M5.22 18.78l1.42-1.42m10.72-10.72 1.42-1.42" />
                                    </svg>
                        ...
                        <div className="flex items-center gap-5 mt-4 pt-3 border-t border-gray-100 justify-center">
                            <LegendItem status="sold" label="Đã bán" />
                            <LegendItem status="available" label="Trống" />
                            <LegendItem status="selecting" label="Đang chọn" />
                        </div>
                ...
                <div className="w-full lg:w-72 flex-shrink-0">
                    <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-5 space-y-4 sticky top-6">
                        ...
                        <div className="flex items-center justify-between py-2 border-t border-gray-100">
                            ...
                            >
                                <span className={`absolute top-0.5 w-5 h-5 bg-white rounded-full shadow transition-all duration-200 ${printAfter ? "left-5" : "left-0.5"}`} />
                            </button>
                        </div>
                        ...
                        <button
                            ...
                            className={`w-full py-3 rounded-xl font-bold text-sm transition-all duration-200 ${selectedSeats.length > 0
