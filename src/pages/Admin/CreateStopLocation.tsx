...
import type { geoLocation, stopLocation } from "../../model/stopLocation";
...
export default function CreateStopLocation() {
    ...
    const [location, setLocation] = useState<geoLocation>({type:"Point",coordinates :[0,0]})
    ...
    const getGeoLocation = async (locationName : string) =>{
        try {
            const res = await baseAPIAuth.post("/api/admin/check/getGeoStopLocation",{
    ...
    const createNewStopLocation = async () =>{
        try {
            const res = await baseAPIAuth.post("/api/admin/check/createStopLocation",{
                stop_id: stopId,
                location_name: locationName,
                address,
                status,
        ...
        <div className="min-h-screen bg-[#f6f7f9] text-[#1f2937]">
            <section className="w-full">
                <div className="mx-auto w-full max-w-[1100px] space-y-6 px-4 pb-16 pt-10 lg:px-4">
                    ...
                    <form
                        className="mt-10 space-y-5 rounded-[20px] border border-[#e7eaf0] bg-white p-5 shadow-[0_16px_36px_-26px_rgba(15,23,42,0.34)]"
                    >
                            ...
                            <div className="grid gap-4 md:grid-cols-2">
                                <label className="space-y-1">
                                    ...
                                    <input
                                        ...
                                        placeholder="Nhập Route_Stop ID hoặc mã node"
                        ...
                        <form onSubmit={handleImportSubmit} className="space-y-4 p-5">
                            ...
                            <label className="flex cursor-pointer flex-col items-center justify-center rounded-xl border border-dashed border-[#d1d5db] bg-[#f9fafb] px-4 py-8 text-center transition hover:border-[#c4cad5] hover:bg-[#f3f4f6]">
                                ...
                                <span className="text-[13px] font-semibold text-[#374151]">
                                    {importFile ? importFile.name : "Chọn file Excel (.xlsx, .xls)"}
                                </span>
